...
function drawCard(x, y, card, isSelected, isBoss, phase, selectedCards, discardCards, health, damage)
    ...
    for _, c in ipairs(discardCards) do
        if c == card then
            isCardSelected = true
            highlightColor = COLORS.discard
            break
        end
    end

    if isCardSelected then
        drawRoundedRect(x - 3, y - 3, cardWidth + 6, cardHeight + 6, radius + 3, highlightColor[1], highlightColor[2],
            highlightColor[3], 0.4)
    end
...
function love.draw()
    ...
    drawRoundedRect(bossX, bossY, battleW - 60, bossAreaHeight - 20, 10, COLORS.panelLight[1], COLORS.panelLight[2],
        COLORS.panelLight[3], 1)
    ...
    if gameState.boss.current then
        ...
        drawCard(bossCardX, bossCardY, gameState.boss.current, false, true, gameState.turn.phase,
            gameState.turn.selectedCards, gameState.turn.discardCards, gameState.boss.health, gameState.turn.bossDamage)
    else
    ...
    for i, card in ipairs(gameState.player.hand) do
        ...
        drawCard(cardX, cardY, card, isSelected, false, gameState.turn.phase, gameState.turn.selectedCards,
            gameState.turn.discardCards)
    end
    ...
    if gameState.turn.phase == rules.TURN_PHASE.PLAYER and #gameState.turn.selectedCards > 0 then
        ...
        love.graphics.print(#gameState.turn.selectedCards .. " cards, " .. total .. " damage", battleX + 145,
            buttonY + 14)
    elseif gameState.turn.phase == rules.TURN_PHASE.BOSS_DAMAGE and #gameState.turn.discardCards > 0 then
        ...
        love.graphics.print(total .. " / " .. gameState.turn.bossDamage .. " (need " .. gameState.turn.bossDamage .. ")",
            battleX + 145, buttonY + 14)
    end
end
